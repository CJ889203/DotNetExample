...
namespace System
{
  ...
  public class Exception : ISerializable
  {
    ...
    [MethodImpl(MethodImplOptions.InternalCall)]
    private static extern IRuntimeMethodInfo GetMethodFromStackTrace(
      object stackTrace);
    ...
    {
      System.Diagnostics.StackTrace stackTrace = new System.Diagnostics.StackTrace(this, false);
      if (stackTrace.FrameCount <= 0)
        return (string) null;
      switch (stackTrace.GetFrame(0).GetMethod().Module)
      {
        case RuntimeModule internalModule:
label_4:
          return internalModule.GetRuntimeAssembly().GetSimpleName();
        case ModuleBuilder moduleBuilder:
          internalModule = (RuntimeModule) moduleBuilder.InternalModule;
          goto label_4;
        default:
    ...
    {
      this._message = info != null ? info.GetString(nameof (Message)) : throw new ArgumentNullException(nameof (info));
    ...
    {
      get => this._source ?? (this._source = this.CreateSourceName());
