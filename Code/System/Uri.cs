...
namespace System
{
  ...
  public class Uri : ISerializable
  {
    ...
    private bool IsUncOrDosPath => (this._flags & (Uri.Flags.DosPath | Uri.Flags.UncPath)) > Uri.Flags.Zero;
    ...
    #nullable disable
    internal static bool IriParsingStatic(UriParser syntax) => syntax == null || syntax.InFact(UriSyntaxFlags.AllowIriParsing);

    internal bool DisablePathAndQueryCanonicalization => (this._flags & Uri.Flags.DisablePathAndQueryCanonicalization) > Uri.Flags.Zero;
    ...
    private static bool StaticNotAny(Uri.Flags allFlags, Uri.Flags checkFlags) => (allFlags & checkFlags) == Uri.Flags.Zero;

    private static bool StaticInFact(Uri.Flags allFlags, Uri.Flags checkFlags) => (allFlags & checkFlags) > Uri.Flags.Zero;
    ...
    {
      string uri = uriString != null ? uriString : throw new ArgumentNullException(nameof (uriString));
    ...
    {
      string uri = uriString != null ? uriString : throw new ArgumentNullException(nameof (uriString));
      int num = dontEscape ? 1 : 0;
      UriCreationOptions uriCreationOptions = new UriCreationOptions();
      ref UriCreationOptions local = ref uriCreationOptions;
      this.CreateThis(uri, num != 0, UriKind.Absolute, in local);
    }
    ...
    public Uri(string uriString, UriKind uriKind)
    {
      ...
      UriCreationOptions uriCreationOptions = new UriCreationOptions();
      ref UriCreationOptions local = ref uriCreationOptions;
      this.CreateThis(uri, false, (UriKind) num, in local);
    }
    ...
    public Uri(Uri baseUri, Uri relativeUri)
    {
      ...
      this._syntax = (UriParser) null;
      this._originalUnicodeString = (string) null;
      string uri = newUriString;
      int num = userEscaped ? 1 : 0;
      UriCreationOptions uriCreationOptions = new UriCreationOptions();
      ref UriCreationOptions local = ref uriCreationOptions;
      this.CreateThis(uri, num != 0, UriKind.Absolute, in local);
    }
      ...
      for (int index = 0; index < relativeStr.Length && relativeStr[index] != '/' && relativeStr[index] != '\\' && relativeStr[index] != '?' && relativeStr[index] != '#'; ++index)
      {
        if (relativeStr[index] == ':')
        {
          if (index >= 2)
          {
            ...
            if (Uri.CheckSchemeSyntax(relativeStr.AsSpan(0, index), ref syntax) == ParsingError.None)
            {
    ...
    private string GetLocalPath()
    {
      ...
      if (this.NotAny(Uri.Flags.HostNotCanonical | Uri.Flags.PathNotCanonical | Uri.Flags.ShouldBeCompressed))
      {
        ...
        string localPath = !this.IsImplicitFile || (int) this._info.Offset.Host != (this.IsDosPath ? 0 : 2) || (int) this._info.Offset.Query != (int) this._info.Offset.End ? (!this.IsDosPath || this._string[num] != '/' && this._string[num] != '\\' ? this._string.Substring(num, (int) this._info.Offset.Query - num) : this._string.Substring(num + 1, (int) this._info.Offset.Query - num - 1)) : this._string;
        if (this.IsDosPath && localPath[1] == '|')
          localPath = localPath.Remove(1, 1).Insert(1, ":");
        for (int index = 0; index < localPath.Length; ++index)
        {
          if (localPath[index] == '/')
          {
      ...
      char[] dest = new char[host.Length + 3 + (int) this._info.Offset.Fragment - (int) this._info.Offset.Path];
      if (this.IsUncPath)
      {
        dest[0] = '\\';
        dest[1] = '\\';
        length = 2;
        UriHelper.UnescapeString(host, 0, host.Length, dest, ref length, char.MaxValue, char.MaxValue, char.MaxValue, UnescapeMode.CopyOnly, this._syntax, false);
      }
      ...
      UriHelper.UnescapeString(this._string, path, (int) this._info.Offset.Query, dest, ref length, char.MaxValue, char.MaxValue, char.MaxValue, unescapeMode, this._syntax, true);
      if (dest[1] == '|')
        dest[1] = ':';
      if (this.InFact(Uri.Flags.ShouldBeCompressed))
        Uri.Compress(dest, this.IsDosPath ? (int) num1 + 2 : (int) num1, ref length, this._syntax);
      for (ushort index = 0; (int) index < (int) (ushort) length; ++index)
      {
        if (dest[(int) index] == '/')
          dest[(int) index] = '\\';
      }
          ...
          case null:
            ...
            switch (this.HostType)
            {
              ...
              case Uri.Flags.BasicHostType:
                if (this.InFact(Uri.Flags.HostNotCanonical | Uri.Flags.E_HostNotCanonical))
                {
                  ValueStringBuilder dest = new ValueStringBuilder(new Span<char>((void*) __untypedstackalloc(new IntPtr(1024)), 512));
    ...
    public static unsafe UriHostNameType CheckHostName(string? name)
    {
      ...
      char* name1 = (char*) num2;
      if (name[0] == '[' && name[name.Length - 1] == ']' && IPv6AddressHelper.IsValid(name1, 1, ref num1) && num1 == name.Length)
        return UriHostNameType.IPv6;
      num1 = name.Length;
      if (IPv4AddressHelper.IsValid(name1, 0, ref num1, false, false, false) && num1 == name.Length)
        return UriHostNameType.IPv4;
      num1 = name.Length;
      bool notCanonical = false;
      if (DomainNameHelper.IsValid(name1, 0, ref num1, ref notCanonical, false) && num1 == name.Length)
        return UriHostNameType.Dns;
      num1 = name.Length;
      notCanonical = false;
      if (DomainNameHelper.IsValidByIri(name1, 0, ref num1, ref notCanonical, false) && num1 == name.Length)
        return UriHostNameType.Dns;
      ...
      if (IPv6AddressHelper.IsValid((char*) name2, 1, ref num1) && num1 == name.Length)
        return UriHostNameType.IPv6;
      // ISSUE: fixed variable is out of scope
      // ISSUE: __unpin statement
      __unpin(chPtr);
    ...
    public static bool IsHexEncoding(string pattern, int index) => pattern.Length - index >= 3 && pattern[index] == '%' && Uri.IsHexDigit(pattern[index + 1]) && Uri.IsHexDigit(pattern[index + 2]);
    ...
    public static bool CheckSchemeName([NotNullWhen(true)] string? schemeName)
    {
      ...
      {
        if (!UriHelper.IsAsciiLetterOrDigit(schemeName[index]) && schemeName[index] != '+' && schemeName[index] != '-' && schemeName[index] != '.')
          return false;
      }
      return true;
    }
    ...
    public static bool IsHexDigit(char character) => HexConverter.IsHexChar((int) character);
    ...
    public override int GetHashCode()
    {
      ...
      Uri.MoreInfo moreInfo = this.EnsureUriInfo().MoreInfo;
      string str = moreInfo.RemoteUrl ?? (moreInfo.RemoteUrl = this.GetParts(UriComponents.HttpRequestUrl, UriFormat.SafeUnescaped));
    ...
    public override string ToString()
    {
      ...
      if (this._info.String == null)
        this._info.String = !this._syntax.IsSimple ? this.GetParts(UriComponents.AbsoluteUri, UriFormat.SafeUnescaped) : this.GetComponentsHelper(UriComponents.AbsoluteUri, (UriFormat) 32767);
      return this._info.String;
    }
    ...
    public override bool Equals([NotNullWhen(true)] object? comparand)
    {
      ...
      string a = moreInfo3.RemoteUrl ?? (moreInfo3.RemoteUrl = this.GetParts(UriComponents.HttpRequestUrl, UriFormat.SafeUnescaped));
      Uri.MoreInfo moreInfo4 = moreInfo2;
      string b = moreInfo4.RemoteUrl ?? (moreInfo4.RemoteUrl = result.GetParts(UriComponents.HttpRequestUrl, UriFormat.SafeUnescaped));
    ...
    {
      int index = uriString.IndexOfAny(Uri.s_pathDelims);
      return index >= 0 && uriString[index] == ':';
    }

    internal static string InternalEscapeString(string rawString) => rawString != null ? UriHelper.EscapeString(rawString, true, UriHelper.UnreservedReservedTable, '?', '#') : string.Empty;

    private static unsafe ParsingError ParseScheme(
      string uriString,
      ref Uri.Flags flags,
      ref UriParser syntax)
    {
      ...
      {
        fixed (char* chPtr = &uriString.GetPinnableReference())
          num = (IntPtr) chPtr;
      }
      char* uriString1 = (char*) num;
      ParsingError err = ParsingError.None;
      int checkImplicitFile = Uri.ParseSchemeCheckImplicitFile(uriString1, length, ref err, ref flags, ref syntax);
    ...
    private unsafe ParsingError PrivateParseMinimal()
    {
      ...
      if (this._syntax.IsAllSet(UriSyntaxFlags.AllowEmptyHost | UriSyntaxFlags.AllowDOSPath) && this.NotAny(Uri.Flags.ImplicitFile) && index1 + 1 < length)
      {
        ...
        if (this._syntax.InFact(UriSyntaxFlags.FileLikeUri) || index2 - index1 <= 3)
        {
          ...
          char ch2;
          if (index2 + 1 < length && ((ch2 = pString[index2 + 1]) == ':' || ch2 == '|') && UriHelper.IsAsciiLetter(pString[index2]))
          {
            char ch3;
      ...
      if (err != ParsingError.None)
        return err;
      if (index3 < length && pString[index3] == '\\' && this.NotAny(Uri.Flags.ImplicitFile) && this._syntax.NotAny(UriSyntaxFlags.AllowDOSPath))
...
label_49:
        ...
        if (this.HostType == Uri.Flags.BasicHostType)
        {
          ...
          if ((check & (Uri.Check.EscapedCanonical | Uri.Check.BackslashInPath)) != Uri.Check.EscapedCanonical)
          {
            ...
            if (this.NotAny(Uri.Flags.UserEscaped))
              stringToEscape = UriHelper.EscapeString(stringToEscape, !this.IsImplicitFile, UriHelper.UnreservedReservedTable, '?', '#');
          }
        }
    ...
    internal string GetParts(UriComponents uriParts, UriFormat formatAs) => this.InternalGetComponents(uriParts, formatAs);
    ...
    private unsafe string ReCreateParts(
      UriComponents parts,
      ushort nonCanonical,
      UriFormat formatAs)
    {
      ...
      ValueStringBuilder dest1 = str1.Length > 512 ? new ValueStringBuilder(str1.Length) : new ValueStringBuilder(new Span<char>((void*) __untypedstackalloc(new IntPtr(1024)), 512));
      if ((parts & UriComponents.Scheme) != (UriComponents) 0)
      {
        dest1.Append(this._syntax.SchemeName);
      ...
      ReadOnlySpan<char> readOnlySpan1;
      if ((parts & UriComponents.UserInfo) != (UriComponents) 0 && this.InFact(Uri.Flags.HasUserInfo))
      {
        ReadOnlySpan<char> readOnlySpan2 = str1.AsSpan((int) this._info.Offset.User, (int) this._info.Offset.Host - (int) this._info.Offset.User);
        if (((int) nonCanonical & 2) != 0)
        {
          switch (formatAs)
          {
            case UriFormat.UriEscaped:
              if (this.NotAny(Uri.Flags.UserEscaped))
              {
                UriHelper.EscapeString(readOnlySpan2, ref dest1, true, '?', '#');
                break;
              }
              this.InFact(Uri.Flags.E_UserNotCanonical);
              dest1.Append(readOnlySpan2);
              break;
            case UriFormat.Unescaped:
              UriHelper.UnescapeString(readOnlySpan2, ref dest1, char.MaxValue, char.MaxValue, char.MaxValue, UnescapeMode.Unescape | UnescapeMode.UnescapeAll, this._syntax, false);
              break;
            case UriFormat.SafeUnescaped:
              readOnlySpan1 = readOnlySpan2;
              int length = readOnlySpan1.Length - 1 - 0;
              UriHelper.UnescapeString(readOnlySpan1.Slice(0, length), ref dest1, '@', '/', '\\', this.InFact(Uri.Flags.UserEscaped) ? UnescapeMode.Unescape : UnescapeMode.EscapeUnescape, this._syntax, false);
              dest1.Append('@');
              break;
            default:
              dest1.Append(readOnlySpan2);
      ...
      if ((parts & UriComponents.Host) != (UriComponents) 0)
      {
        ...
        if (str2.Length != 0)
        {
          ...
          // ISSUE: untyped stack allocation
          ValueStringBuilder dest2 = new ValueStringBuilder(new Span<char>((void*) __untypedstackalloc(new IntPtr(1024)), 512));
          if ((parts & UriComponents.NormalizedHost) != (UriComponents) 0)
          {
            str2 = UriHelper.StripBidiControlCharacters((ReadOnlySpan<char>) str2, str2);
            if (!DomainNameHelper.TryGetUnicodeEquivalent(str2, ref dest2))
              dest2.Length = 0;
          }
          UriHelper.UnescapeString(dest2.Length == 0 ? (ReadOnlySpan<char>) str2 : dest2.AsSpan(), ref dest1, '/', '?', '#', unescapeMode, this._syntax, false);
          dest2.Dispose();
          ...
          {
            --dest1.Length;
            dest1.Append(this._info.ScopeId);
            dest1.Append(']');
          }
        }
      }
      ...
      {
        dest1.Append(':');
        ref ushort local1 = ref this._info.Offset.PortValue;
        Span<char> destination = dest1.AppendSpan(5);
        int num;
        ref int local2 = ref num;
        readOnlySpan1 = new ReadOnlySpan<char>();
        ReadOnlySpan<char> format = readOnlySpan1;
        local1.TryFormat(destination, out local2, format);
        dest1.Length -= 5 - num;
      }
      if ((parts & UriComponents.Path) != (UriComponents) 0)
      {
        this.GetCanonicalPath(ref dest1, formatAs);
        if (parts == UriComponents.Path)
        {
          int start = !this.InFact(Uri.Flags.AuthorityFound) || dest1.Length == 0 || dest1[0] != '/' ? 0 : 1;
          readOnlySpan1 = dest1.AsSpan(start);
          string parts1 = readOnlySpan1.ToString();
          dest1.Dispose();
          return parts1;
        }
      }
        ...
        if (((int) nonCanonical & 32) != 0)
        {
          ...
          switch (formatAs)
          {
            case UriFormat.UriEscaped:
              if (this.NotAny(Uri.Flags.UserEscaped))
              {
                UriHelper.EscapeString(str1.AsSpan(start, (int) this._info.Offset.Fragment - start), ref dest1, true, '#');
                goto label_40;
              }
              else
                goto label_39;
...
label_39:
        UriHelper.UnescapeString(str1, start, (int) this._info.Offset.Fragment, ref dest1, '#', char.MaxValue, char.MaxValue, unescapeMode1, this._syntax, true);
      }
label_40:
        ...
        if (((int) nonCanonical & 64) != 0)
        {
          ...
          switch (formatAs)
          {
            case UriFormat.UriEscaped:
              if (this.NotAny(Uri.Flags.UserEscaped))
              {
                UriHelper.EscapeString(str1.AsSpan(start, (int) this._info.Offset.End - start), ref dest1, true);
                goto label_52;
              }
              else
                goto label_51;
...
label_51:
        UriHelper.UnescapeString(str1, start, (int) this._info.Offset.End, ref dest1, '#', char.MaxValue, char.MaxValue, unescapeMode3, this._syntax, false);
      }
label_52:
      return dest1.ToString();
    }
    ...
    private unsafe void ParseRemaining()
    {
      ...
      if (!this.UserDrivenParsing)
      {
        ...
        char* str2 = (char*) num1;
        this.GetLengthWithoutTrailingSpaces(this._string, ref length1, scheme);
        ...
        {
          if (flag1)
            this._string += this._originalUnicodeString.Substring(num2);
        ...
        else
        {
          if (flag1)
          {
            ...
            if (this._string == this._originalUnicodeString)
              this.GetLengthWithoutTrailingSpaces(this._string, ref length1, path);
          }
    ...
    private static unsafe int ParseSchemeCheckImplicitFile(
      char* uriString,
      int length,
      ...
      ref UriParser syntax)
    {
      int checkImplicitFile1 = 0;
      while (checkImplicitFile1 < length && UriHelper.IsLWS(uriString[checkImplicitFile1]))
        ++checkImplicitFile1;
      int index = checkImplicitFile1;
      while (index < length && uriString[index] != ':')
        ++index;
      if (IntPtr.Size != 4)
        ;
      if (checkImplicitFile1 + 2 >= length || index == checkImplicitFile1)
      {
        err = ParsingError.BadFormat;
        return 0;
      }
      char ch1;
      if ((ch1 = uriString[checkImplicitFile1 + 1]) == ':' || ch1 == '|')
      {
        if (UriHelper.IsAsciiLetter(uriString[checkImplicitFile1]))
        {
          char ch2;
          if ((ch2 = uriString[checkImplicitFile1 + 2]) == '\\' || ch2 == '/')
          {
            flags |= Uri.Flags.AuthorityFound | Uri.Flags.DosPath | Uri.Flags.ImplicitFile;
            syntax = UriParser.FileUri;
            return checkImplicitFile1;
          }
          err = ParsingError.MustRootedPath;
          return 0;
        }
        err = ch1 != ':' ? ParsingError.BadFormat : ParsingError.BadScheme;
        return 0;
      }
      char ch3;
      if ((ch3 = uriString[checkImplicitFile1]) == '/' || ch3 == '\\')
      {
        char ch4;
        if ((ch4 = uriString[checkImplicitFile1 + 1]) == '\\' || ch4 == '/')
        {
          flags |= Uri.Flags.AuthorityFound | Uri.Flags.UncPath | Uri.Flags.ImplicitFile;
          syntax = UriParser.FileUri;
          int checkImplicitFile2 = checkImplicitFile1 + 2;
          char ch5;
          while (checkImplicitFile2 < length && ((ch5 = uriString[checkImplicitFile2]) == '/' || ch5 == '\\'))
            ++checkImplicitFile2;
          return checkImplicitFile2;
        }
        err = ParsingError.BadFormat;
        return 0;
      }
      ...
      err = Uri.CheckSchemeSyntax(new ReadOnlySpan<char>((void*) (uriString + checkImplicitFile1), index - checkImplicitFile1), ref syntax);
      return err != ParsingError.None ? 0 : index + 1;
    }

    private static unsafe ParsingError CheckSchemeSyntax(
      ...
      ref UriParser syntax)
    {
      ...
      if ((uint) ch1 - 65U <= 25U)
        ch1 |= ' ';
      else if ((uint) ch1 - 97U > 25U)
        return ParsingError.BadScheme;
      switch (span.Length)
      {
        case 2:
          if (30579 == ((int) ch1 << 8 | (int) ToLowerCaseAscii(span[1])))
          {
          ...
          break;
        case 3:
          switch ((int) ch1 << 16 | (int) ToLowerCaseAscii(span[1]) << 8 | (int) ToLowerCaseAscii(span[2]))
          {
            case 6714480:
          ...
          break;
        case 4:
          switch ((int) ch1 << 24 | (int) ToLowerCaseAscii(span[1]) << 16 | (int) ToLowerCaseAscii(span[2]) << 8 | (int) ToLowerCaseAscii(span[3]))
          {
            case 1718185061:
              syntax = UriParser.FileUri;
              return ParsingError.None;
          ...
          break;
        case 5:
          if (1752462448 == ((int) ch1 << 24 | (int) ToLowerCaseAscii(span[1]) << 16 | (int) ToLowerCaseAscii(span[2]) << 8 | (int) ToLowerCaseAscii(span[3])) && ToLowerCaseAscii(span[4]) == 's')
          {
          ...
          break;
        case 6:
          if (1835100524 == ((int) ch1 << 24 | (int) ToLowerCaseAscii(span[1]) << 16 | (int) ToLowerCaseAscii(span[2]) << 8 | (int) ToLowerCaseAscii(span[3])) && ToLowerCaseAscii(span[4]) == 't' && ToLowerCaseAscii(span[5]) == 'o')
          {
      ...
      {
        char ch2 = span[index];
        if ((uint) ch2 - 97U > 25U && (uint) ch2 - 65U > 25U)
        {
          switch (ch2)
          {
            case '+':
      ...
      fixed (char* chPtr = &span.GetPinnableReference())
        lwrCaseScheme = string.Create<(IntPtr, int)>(span.Length, ((IntPtr) (void*) chPtr, span.Length), (SpanAction<char, (IntPtr, int)>) ((buffer, state) => new ReadOnlySpan<char>((void*) state.ip, state.length).ToLowerInvariant(buffer)));
      syntax = UriParser.FindOrFetchAsUnknownV1Syntax(lwrCaseScheme);
      return ParsingError.None;

      static char ToLowerCaseAscii(char c) => (uint) c - 65U > 25U ? c : (char) ((uint) c | 32U);
    }
      ...
      ref string newHost)
    {
      ...
      char ch;
      if (idx == length || (ch = pString[idx]) == '/' || ch == '\\' && Uri.StaticIsFile(syntax) || ch == '#' || ch == '?')
      {
        ...
        for (; index < end; ++index)
        {
          ...
          if (pString[index] == '@')
          {
            ...
            if (flag1)
            {
              if (flag2)
              {
                string str2 = IriHelper.EscapeUnescapeIri(pString, num1, index + 1, UriComponents.UserInfo);
                newHost += str2;
                ...
                str1 = new string(pString, num1, index - num1 + 1);
            }
            ++index;
            ch = pString[index];
            break;
      ...
      bool notCanonical = (flags1 & UriSyntaxFlags.SimpleUserSyntax) == UriSyntaxFlags.None;
      if (ch == '[' && syntax.InFact(UriSyntaxFlags.AllowIPv6Host) && IPv6AddressHelper.IsValid(pString, index + 1, ref end))
      {
        ...
        if (flag2)
        {
          newHost += new string(pString, index, end - index);
          flags |= Uri.Flags.HostUnicodeNormalized;
          justNormalized = true;
        }
      }
      else if (ch <= '9' && ch >= '0' && syntax.InFact(UriSyntaxFlags.AllowIPv4Host) && IPv4AddressHelper.IsValid(pString, index, ref end, false, Uri.StaticNotAny(flags, Uri.Flags.ImplicitFile), syntax.InFact(UriSyntaxFlags.V1_UnknownUri)))
      {
      ...
      else if ((flags1 & UriSyntaxFlags.AllowDnsHost) != UriSyntaxFlags.None && !flag1 && DomainNameHelper.IsValid(pString, index, ref end, ref notCanonical, Uri.StaticNotAny(flags, Uri.Flags.ImplicitFile)))
      {
        flags |= Uri.Flags.DnsHostType;
        if (!notCanonical)
          flags |= Uri.Flags.CanonicalDnsHost;
      }
      else if ((flags1 & UriSyntaxFlags.AllowDnsHost) != UriSyntaxFlags.None && (flag2 || syntax.InFact(UriSyntaxFlags.AllowIdn)) && DomainNameHelper.IsValidByIri(pString, index, ref end, ref notCanonical, Uri.StaticNotAny(flags, Uri.Flags.ImplicitFile)))
        this.CheckAuthorityHelperHandleDnsIri(pString, index, end, hasUnicode, ref flags, ref justNormalized, ref newHost, ref err);
      else if ((flags1 & UriSyntaxFlags.AllowUncHost) != UriSyntaxFlags.None && UncNameHelper.IsValid(pString, index, ref end, Uri.StaticNotAny(flags, Uri.Flags.ImplicitFile)) && end - index <= 256)
      {
        ...
        if (syntax.InFact(UriSyntaxFlags.MayHavePort))
        {
          ...
          for (idx = end + 1; idx < length; ++idx)
          {
            ...
            switch (num3)
            {
              ...
              case 8:
              case 9:
                if ((num2 = num2 * 10 + num3) <= (int) ushort.MaxValue)
                  continue;
                goto label_51;
              default:
...
label_51:
          ...
          if (hasUnicode & justNormalized)
            newHost += new string(pString, startIndex, idx - startIndex);
        }
        else
          flags &= ~Uri.Flags.HostTypeMask;
      }
      if ((flags & Uri.Flags.HostTypeMask) == Uri.Flags.Zero)
      {
        ...
        if (syntax.InFact(UriSyntaxFlags.AllowAnyOtherHost))
        {
          ...
          if (flag2)
          {
            string str3 = new string(pString, num1, end - num1);
            try
            {
              newHost += str3.Normalize(NormalizationForm.FormC);
            }
        ...
        else if (syntax.InFact(UriSyntaxFlags.V1_UnknownUri))
        {
          ...
          if (flag2)
          {
            string str4 = new string(pString, startIndex, end - startIndex);
            try
            {
              newHost += str4.Normalize(NormalizationForm.FormC);
            }
            catch (ArgumentException ex)
            {
              err = ParsingError.BadFormat;
    ...
    private unsafe void CheckAuthorityHelperHandleDnsIri(
      char* pString,
      int start,
      int end,
      bool hasUnicode,
    ...
    private unsafe void GetCanonicalPath(ref ValueStringBuilder dest, UriFormat formatAs)
    {
      ...
      if (formatAs == UriFormat.UriEscaped)
      {
        ...
        else if (this.InFact(Uri.Flags.E_PathNotCanonical) && this.NotAny(Uri.Flags.UserEscaped))
        {
          ...
          UriHelper.EscapeString(readOnlySpan.Slice((int) this._info.Offset.Path, (int) this._info.Offset.Query - (int) this._info.Offset.Path), ref dest, !this.IsImplicitFile, '?', '#');
        }
        else
          dest.Append(this._string.AsSpan((int) this._info.Offset.Path, (int) this._info.Offset.Query - (int) this._info.Offset.Path));
      }
      ...
      if (this.InFact(Uri.Flags.ShouldBeCompressed) && dest.Length - start > 0)
      {
        ...
        if (formatAs == UriFormat.UriEscaped && this.NotAny(Uri.Flags.UserEscaped) && this.InFact(Uri.Flags.E_PathNotCanonical))
        {
          ...
          dest.Length = length1;
          UriHelper.EscapeString(MemoryMarshal.CreateReadOnlySpan<char>(ref valueStringBuilder.GetPinnableReference(), valueStringBuilder.Length), ref dest, !this.IsImplicitFile, '?', '#');
          length1 = dest.Length;
      ...
      if (unescapeMode == UnescapeMode.CopyOnly)
        return;
      // ISSUE: untyped stack allocation
      ValueStringBuilder valueStringBuilder1 = new ValueStringBuilder(new Span<char>((void*) __untypedstackalloc(new IntPtr(1024)), 512));
    ...
    private static void Compress(char[] dest, int start, ref int destLength, UriParser syntax) => destLength = start + Uri.Compress(dest.AsSpan<char>(start, destLength - start), syntax);
      ...
      for (int index = span.Length - 1; index >= 0; --index)
      {
        ...
        else
        {
          if (num3 != 0)
          {
            ...
            {
              int start = index + 1 + num3 + (num2 == 0 ? 0 : 1);
    ...
    private static string CombineUri(Uri basePart, string relativePart, UriFormat uriFormat)
    {
      ...
      if (Uri.StaticIsFile(basePart.Syntax) && (ch1 == '\\' || ch1 == '/'))
      {
        ...
        for (int index = 1; index < readOnlySpan.Length; ++index)
        {
          ...
          {
            readOnlySpan = readOnlySpan.Slice(0, index);
            break;
          }
        }
      ...
      else if (basePart.IsImplicitFile)
      {
        ...
        str1 = "\\\\" + basePart.GetParts(UriComponents.Host, UriFormat.Unescaped);
      }
      else
    ...
    protected static string EscapeString(string? str) => str != null ? UriHelper.EscapeString(str, true, UriHelper.UnreservedReservedTable, '?', '#') : string.Empty;
    ...
    protected virtual bool IsReservedCharacter(char character) => character == ';' || character == '/' || character == ':' || character == '@' || character == '&' || character == '=' || character == '+' || character == '$' || character == ',';
    ...
    protected static bool IsExcludedCharacter(char character) => character <= ' ' || character >= '\u007F' || character == '<' || character == '>' || character == '#' || character == '%' || character == '"' || character == '{' || character == '}' || character == '|' || character == '\\' || character == '^' || character == '[' || character == ']' || character == '`';
    ...
    protected virtual bool IsBadFileSystemCharacter(char character) => character < ' ' || character == ';' || character == '/' || character == '?' || character == ':' || character == '&' || character == '=' || character == ',' || character == '*' || character == '<' || character == '>' || character == '"' || character == '|' || character == '\\' || character == '^';
    ...
    private void InitializeUri(ParsingError err, UriKind uriKind, out UriFormatException e)
    {
          ...
          if (this.NotAny(Uri.Flags.DosPath))
          {
            switch (uriKind)
            {
              ...
              case UriKind.Relative:
                ...
                return;
              default:
                if (this._string.Length < 2 || this._string[0] == '\\' && this._string[1] == '\\')
                  break;
      ...
      else if (err != ParsingError.None && uriKind != UriKind.Absolute && err <= ParsingError.EmptyUriString)
      {
        ...
        if (this._string.Length <= (int) ushort.MaxValue)
          return;
        err = ParsingError.SizeLimit;
      }
      else
    ...
    private static bool CheckForUnicodeOrEscapedUnreserved(string data)
    {
      for (int index1 = 0; index1 < data.Length; ++index1)
      {
        ...
        if (ch == '%')
        {
          if ((uint) (index1 + 2) < (uint) data.Length)
          {
            char index2 = UriHelper.DecodeHexChars((int) data[index1 + 1], (int) data[index1 + 2]);
            int num = (int) index2;
            ReadOnlySpan<bool> unreservedTable = UriHelper.UnreservedTable;
            int length = unreservedTable.Length;
            if (num < length)
            {
              unreservedTable = UriHelper.UnreservedTable;
              if (!unreservedTable[(int) index2])
              {
                index1 += 2;
                continue;
              }
    ...
    public static bool TryCreate([NotNullWhen(true)] string? uriString, UriKind uriKind, [NotNullWhen(true)] out Uri? result)
    {
      ...
      UriFormatException uriFormatException = (UriFormatException) null;
      ref Uri local1 = ref result;
      string uriString1 = uriString;
      int num = (int) uriKind;
      ref UriFormatException local2 = ref uriFormatException;
      UriCreationOptions uriCreationOptions = new UriCreationOptions();
      ref UriCreationOptions local3 = ref uriCreationOptions;
      Uri helper = Uri.CreateHelper(uriString1, false, (UriKind) num, ref local2, in local3);
      local1 = helper;
      return uriFormatException == null && result != (Uri) null;
    }
    ...
    public static bool TryCreate(Uri? baseUri, Uri? relativeUri, [NotNullWhen(true)] out Uri? result)
    {
      ...
      if ((object) result == null)
      {
        ...
        int num = userEscaped ? 1 : 0;
        ref UriFormatException local2 = ref parsingError;
        UriCreationOptions uriCreationOptions = new UriCreationOptions();
        ref UriCreationOptions local3 = ref uriCreationOptions;
        Uri helper = Uri.CreateHelper(uriString, num != 0, UriKind.Absolute, ref local2, in local3);
        local1 = helper;
      }
    ...
    public string GetComponents(UriComponents components, UriFormat format) => !this.DisablePathAndQueryCanonicalization || (components & UriComponents.PathAndQuery) == (UriComponents) 0 ? this.InternalGetComponents(components, format) : throw new InvalidOperationException(SR.net_uri_GetComponentsCalledWhenCanonicalizationDisabled);
    ...
    public bool IsWellFormedOriginalString() => this.IsNotAbsoluteUri || this.Syntax.IsSimple ? this.InternalIsWellFormedOriginalString() : this.Syntax.InternalIsWellFormedOriginalString(this);
    ...
    public static string EscapeUriString(string stringToEscape) => UriHelper.EscapeString(stringToEscape, false, UriHelper.UnreservedReservedTable);
    ...
    /// The length of <paramref name="stringToEscape" /> exceeds 32766 characters.</exception>
    /// <returns>The escaped representation of <paramref name="stringToEscape" />.</returns>
    public static string EscapeDataString(string stringToEscape) => UriHelper.EscapeString(stringToEscape, false, UriHelper.UnreservedTable);
      ...
      ref bool userEscaped)
    {
      ...
      if (relativeStr.Length >= 3 && (relativeStr[1] == ':' || relativeStr[1] == '|') && UriHelper.IsAsciiLetter(relativeStr[0]) && (relativeStr[2] == '\\' || relativeStr[2] == '/'))
      {
    ...
    private unsafe string GetRelativeSerializationString(UriFormat format)
    {
      ...
      {
        case UriFormat.UriEscaped:
          return UriHelper.EscapeString(this._string, true, UriHelper.UnreservedReservedTable);
        case UriFormat.Unescaped:
    ...
    internal unsafe bool IsBaseOfHelper(Uri uriLink)
    {
      ...
      if (!uriLink.IsAbsoluteUri)
      {
        ...
        if ((object) uriLink == null)
        {
          ...
          string uriString = newUriString;
          int num = userEscaped ? 1 : 0;
          ref UriFormatException local1 = ref uriFormatException;
          UriCreationOptions uriCreationOptions = new UriCreationOptions();
          ref UriCreationOptions local2 = ref uriCreationOptions;
          uriLink = Uri.CreateHelper(uriString, num != 0, UriKind.Absolute, ref local1, in local2);
          if (uriFormatException != null)
            return false;
        }
      }
